    ...
    public static String map2json(Map<?, ?> map) {  
        ...
        if (map != null && map.size() > 0) {  
            for (Object key : map.keySet()) {  
                json.append("validateMokey");  
                json.append(":");  
                json.append(object2json(map.get(key)));  //object2json(key)+
                json.append(",");  
            }  
            json.setCharAt(json.length() - 1, '}');  
        } else {  
...
//        userList.add(new UserTest("linbo","北京服装学院",25));  
//        StringBuilder sb = new StringBuilder();  
//        System.out.println(list2json(userList));  
//    }  
}